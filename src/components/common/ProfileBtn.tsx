...
export const ProfileBtn = () => {
  ...
  const [isProvider, setIsProvider] = createSignal<boolean>(false);
  ...
  const providerMode = ((e: Event) => {
    console.log("isUser: ", isUser())
    console.log("localStorage: ", localStorage.getItem('providerMode'))

    e.preventDefault();

    if(isUser() && localStorage.getItem('providerMode') === 'no') {
      localStorage.setItem('providerMode', 'yes');
      setIsProvider(true);
      return true;
    }

    if(isUser() && localStorage.getItem('providerMode') === 'yes') {
      localStorage.setItem('providerMode', 'no');
      setIsProvider(false);
      return false;
    }
  ...
  function switchProviderMode() {
    let test = localStorage.getItem('providerMode');
    ...
    if (isUser()) {
      return (
        <div class="border-2 border-green-300">
          ...
          {/* <div class="my-2" onClick={ () => switchProviderMode()}> */}
          <div class="my-2" onClick={ (e) => providerMode(e)}>
    ...
    <div class="border-2 border-yellow-300">
      ...
      <div>
        <ul id="profileItems" class="hidden fixed z-50 right-2 bg-background1 dark:bg-background1-DM m-2 p-2 rounded-lg justify-start shadow-md shadow-shadow-LM dark:shadow-shadow-DM">
          {/* {renderWhenUser()} */}
          <div class="border-4 border-orange-300">
